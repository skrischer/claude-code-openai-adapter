...
**Important:** The OpenClaw provider system handles **authentication**, not **API execution**.
...
2. Would require modifying OpenClaw core
...
```
OpenClaw → HTTP POST → Local Wrapper Server → Claude CLI subprocess
                                    ↓
OpenClaw ← HTTP Response ← Local Wrapper Server ← Claude CLI stdout
```
...
3. Configure OpenClaw to use this local endpoint
