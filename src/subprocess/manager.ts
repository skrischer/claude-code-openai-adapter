...
import { isAssistantMessage, isResultMessage, isContentDelta, hasToolUse, isToolResult } from "../types/claude-cli.js";
...
export class ClaudeSubprocess extends EventEmitter {
  ...
  private buildArgs(options: SubprocessOptions): string[] {
    ...
    // Optionally skip permission prompts (useful for headless service mode)
    ...
    for (const line of lines) {
      ...
      try {
        ...
        if (isContentDelta(message)) {
          ...
          // Emit separate event for tool_use blocks (for tool activity streaming)
          if (hasToolUse(message)) {
            this.emit("tool_use", message);
          }
        } else if (isToolResult(message)) {
          this.debug(`[Response] Tool result for:`, message.message.content[0]?.tool_use_id);
