...
This plugin enables OpenClaw to use Claude Max subscription through the Claude Code CLI, bypassing the OAuth token scope restrictions that block direct API access.
...
interface SessionMapping {
  openclawId: string;      // OpenClaw conversation ID
...
class SessionManager {
  ...
  getOrCreate(openclawId: string): string {
    const existing = this.sessions.get(openclawId);
    if (existing) {
...
- For OpenClaw integration, we should:
...
3. **Session Isolation**: Each OpenClaw conversation maps to separate Claude session
...
2. Package: `npm pack`
3. Install in OpenClaw: `openclaw plugins install ./claude-code-cli-provider-1.0.0.tgz`
4. Configure: `openclaw models auth login --provider claude-code-cli`
5. Set default model: Edit `~/.openclaw/openclaw.json`

## Future Enhancements

1. **Tool Bridging**: Expose Claude Code's tools (Bash, Read, etc.) to OpenClaw
2. **Cost Tracking**: Track subscription usage across OpenClaw conversations
